...
def session_employee(fieldnames: list[str]) -> frappe._dict | None:
	...
	`commons.safer_permissions`. The `user_id` filter narrows the query to
...
def session_employee_access() -> str:
	...
	is caught by this app's own gate in `commons.safer_permissions` -- was
...
def get_session_user() -> dict:
	...
	"""
	from commons.www.commons import get_user_info
