"""Where NepalERP's education customisations live now that that app is retired.

Almost nothing in this package is a file, and that is the point of it.

`Education Extensions` is a module a site filled in through the desk: twelve
Custom DocTypes -- `Assessment`, `Assessment Marking`, `Course Grade`,
`Financial Aid Application`, `Student Reflection` and their child tables -- plus
three Reports, three Print Formats, a Web Form, and the Server and Client
Scripts that drive them. None of it ships as source. They are `custom = 1`
records in the database, and the only thing tying them to an app is the
`module` field naming a `Module Def` that an app claims.
...
module and `NepalERP` beside it, because `frappe.installer.remove_app` deletes
every doctype under every `Module Def` its app owns -- custom ones included, as
its own confirmation prompt says -- and that was several thousand rows of
assessment and financial-aid history.

So the module was moved rather than the data: `Module Def.app_name` says
`commons`, this app's `modules.txt` names it, and uninstalling NepalERP finds
nothing of its own left to delete.

The name is kept deliberately. It describes what the module holds, a rename
would have to travel through every record that names it, and the module is a
site's own configuration rather than anything this app ships -- so what it is
