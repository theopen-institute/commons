 ...
 *   its boot and this app asks `commons.search.desk_doctypes` for;
 ...
 * `commons.search.desk_doctypes`. Fetched once, lazily, the first time the bar
...
export function ensureDeskDoctypes(): Promise<DeskDoctypes> {
  ...
  if (!deskDoctypesPromise) {
    deskDoctypesPromise = call<DeskDoctypes>('commons.search.desk_doctypes')
      .then((found) => {
