    ...
    <table class="w-full text-left">
      <thead class="bg-surface-gray-2">
        <tr class="text-p-sm text-ink-gray-6">
          ...
          <th class="hidden px-3 py-2 font-normal sm:table-cell">Required by</th>
      ...
      <tbody>
        ...
        >
          <td class="px-3 py-2">
            ...
            <!-- No item code is normal here, not an error: the whole point of
                 a Procurement Request is asking for something the item master
                 does not carry yet. Say so plainly so an approver knows a
                 Material Request cannot be raised until it is coded. -->
            <div class="mt-0.5 text-p-sm text-ink-gray-5">
              <span v-if="line.item_code">{{ line.item_code }}</span>
              <span v-else class="italic">Not in the item catalogue</span>
          ...
          <td class="whitespace-nowrap px-3 py-2 text-right text-base text-ink-gray-7">
            ...
            <div v-if="line.ordered_qty" class="text-p-sm text-ink-gray-5">
              {{ line.ordered_qty }} ordered<template v-if="line.pending_qty">
                · {{ line.pending_qty }} to go</template
              >
            </div>
          </td>
          <td class="hidden whitespace-nowrap px-3 py-2 text-base text-ink-gray-7 sm:table-cell">
            {{ formatDate(line.schedule_date) }}
          </td>
          <td class="whitespace-nowrap px-3 py-2 text-right text-base text-ink-gray-7">
            {{
