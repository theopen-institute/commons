  ...
  <Sidebar class="app-sidebar gap-3.5 border-r p-2 pb-2.5" width="var(--sidebar-width)">
    ...
    <div class="flex-1 space-y-px overflow-y-auto">
      ...
      <template v-else-if="currentApp.key === 'procurement'">
        ...
        />
        <SidebarItem
          v-if="procurementCan.approve"
          label="Approvals"
          icon="lucide-check-check"
          :to="{ name: 'ProcurementApprovals' }"
        >
          <template v-if="procurementCan.pending_approvals" #suffix>
            <Badge theme="amber" variant="subtle">
              {{ procurementCan.pending_approvals }}
            </Badge>
          </template>
        </SidebarItem>
      </template>
    ...
    <a
      :href="userDeskUrl"
      class="app-sidebar__hover mt-3 block shrink-0 rounded-4 px-2 py-1"
    >
...
const RELOAD_SHORTCUT = /mac/i.test(navigator.platform)
  ? '⇧⌘R'
  : 'Shift+Ctrl+R'
