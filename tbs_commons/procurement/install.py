"""Install/migrate hooks for procurement: Custom Fields, tracking and the Workflow.

Everything this module asserts lives on ERPNext's doctypes -- `Material Request`
and its item rows -- or on `Procurement Request`'s own Workflow. None of it is
shared with the rest of the app, so it is wired straight into `hooks.py` the way
