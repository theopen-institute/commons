...
and it makes the boundary between "extends Frappe" and "is about TBS" something
...
`tbs_commons.install.sync_module_defs` already creates `Module Def` records for
modules added to an app after it was installed.

The browser halves stay under `tbs_commons/public/js/`, because that is the only
